...
function Request(method, url) {
  ...
  this._agent = http.globalAgent; //MANU: changed to default: pooling on
  this.method = method;
  this.url = url;
  this.header = {};
  this.writable = true;
  ...
  this.attachmentSize(function(total){
    function next() {
      ...
      var stream = fs.createReadStream(file.path);
        //MANU HACK
      var encoding=self.get("Content-Encoding");
      if(encoding == "gzip"){
          var level=self.get("solvview-zip-level");
          if(level || level == 0 || level == "0") {
              level = parseInt(level);
          } else level = 3; //DEFAULT LEVEL
          var zip=zlib.createGzip({level: level});
          stream=stream.pipe(zip);
      }
        //END MANU HACK
      // TODO: pipe
      // TODO: handle errors
